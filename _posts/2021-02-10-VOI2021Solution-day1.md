...
Cho một hoán vị $$a$$ gồm $$n$$ phần tử. Cần chọn ra $$b$$ là dãy con của $$a$$ gồm $$ 2 \times m$$ số sao cho $$|b[i] - b[i + m]| \le D, \forall 1 \le i \le m$$ và $$m$$ lớn nhất.
...
$$F[i][j] = \begin{cases} max(F[i - 1][j], F[i][j - 1]), \text{if} \space |a[i] - a[j]| > D \\
F[i - 1][j - 1] + 1, \text{if} \space |a[i] - a[j]| \le D \end{cases}$$
...
Ta có thể thấy với mỗi $$i$$, chỉ có nhiều nhất $$2 * D$$ chỉ số $$j$$ thoả mãn $$|a[i] - a[j]| \le D$$ (vì $$a$$ là hoán vị). Ta sẽ tận dụng điều này để tối ưu thuật toán. 
